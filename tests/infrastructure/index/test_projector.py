...
def test_upsert_repair_individual(conn: sqlite3.Connection, storage_repos) -> None:
...
def test_upsert_image_individual(conn: sqlite3.Connection) -> None:
    projector = SqliteIndexProjector(conn)
    repair_id = ULID()
...
def test_upsert_image_with_no_captured_at(conn: sqlite3.Connection) -> None:
    projector = SqliteIndexProjector(conn)
    repair_id = ULID()
...
def test_remove_repair_individual(conn: sqlite3.Connection) -> None:
    projector = SqliteIndexProjector(conn)
    repair_id = ULID()
    repair = Repair(
        id=repair_id,
...
def test_remove_image_individual(conn: sqlite3.Connection) -> None:
    projector = SqliteIndexProjector(conn)
    img = _image(ULID())
    projector.upsert_image(img)
...
def test_full_rebuild_indexes_images(conn: sqlite3.Connection, storage_repos) -> None:
    vrepo, rrepo, irepo, _ = storage_repos
    v = _vehicle("12345")
    vrepo.save(v)
    repair = _repair(v.id, day=1)
